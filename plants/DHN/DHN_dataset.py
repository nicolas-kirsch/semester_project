...
class DHNDataset(CostumDataset):
    def __init__(self, random_seed, horizon, state_dim,cp,mass,xmin= None,xmax=None,umin=None,umax=None, disturbance = {'type':'normal noise'}):
    ...
    def _generate_data(self, num_samples, mode):
        ...
        # flat_heat_demand1 += flat_heat_demand1
        # flat_heat_demand2 =  [30, 20, 25, 30, 35, 40, 50, 60, 70, 80, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]
        # flat_heat_demand2 += flat_heat_demand2
        pad_width = window_size // 2
        # flat_padded = np.pad(flat_heat_demand1, pad_width, mode='edge')
        # smoothed_heat_demand = -np.convolve(flat_padded, np.ones(window_size) / window_size, mode='same')  #valid 


        smoothed_heat_demand = -np.convolve(heat_demand, np.ones(window_size) / window_size, mode='same')  #valid 


        #Initial temperature (randomized)
        # data_x0 = ((self.xmin + (self.xmax-self.xmin)*torch.rand(n_data_total, n_states).to(device)))
        # if mode == "train":
        #     data_x0 = ((30 - 25 + (36-30)*torch.rand(n_data_total, n_states).to(device)))
        # elif mode == "test":
        #     data_x0 = ((self.xmin + (self.xmax-self.xmin)*torch.rand(n_data_total, n_states).to(device)))
        # else:
        #     raise ValueError('A very specific bad thing happened (wrong mode, "train" or "test").') 
        data_x0 = ((self.xmin + (self.xmax-self.xmin)*torch.rand(n_data_total, n_states).to(device)))
